...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  // 1. Get isLoaded to check if data is ready
  const { user, isLoaded } = useUser();
  const { signOut, openUserProfile } = useClerk()

  // 2. If data is not loaded yet, show nothing (or a skeleton/spinner)
  // This prevents the "Undefined" crash on refresh
  if (!isLoaded || !user) {
    return (
      <div className={`w-60 bg-white border-r border-gray-200 flex flex-col items-center justify-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'}`}>
        <div className="animate-pulse w-full h-full bg-gray-50"></div>
      </div>
    )
  }
  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out `}>
      <div className='my-7 w-full'>
        {/* 3. Used optional chaining (?.) just in case */}
